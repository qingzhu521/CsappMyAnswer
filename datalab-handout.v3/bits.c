 ...
 */
int bitCount(int x) {
  //int count = (x&1) + (x>>1&1) + (x>>2&1) + (x>>3&1) + (x>>4&1) + (x>>5&1) + (x>>6&1) + (x>>7&1);
  //para5 0x0000ffff
  //para4 0x00ff00ff
  //para3 0x0f0f0f0f
  //para2 0x33333333
...
int fitsBits(int x, int n) {
  ...
  // int tmp2 = ~(n & tmp1);
  // int tmp3 = tmp2 & n;
  // int minusone = tmp3 | ~(tmp2 + 1 );
  int minusone = n + (~1) + 1; 
  return !(x>>(minusone)) | !(~(x>>(minusone)));
}
/* 
...
int isLessOrEqual(int x, int y) {
  int z = x + (~y) + 1;
  return ((z>>31) & 1 & !((~x)>>31 & (y>>31) & 1)) | !z | (x>>31 & (~y>>31) & 1);
}
/*
